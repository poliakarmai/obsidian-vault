...
## ⚠️ ПРАВИЛО: сначала MCP, потом отчёт
Перед любым отчётом о позициях/PnL — дёрнуть `mcp_bybit_ws_get_positions()`. Vault — кеш, MCP — SSOT.
Не повторять: 03.07 слил «10 позиций -$124» из vault, реально было 5 позиций -$95.68.
...
## ⚠️ ПРАВИЛО 03.07.2026: перед любым отчётом о позициях/PnL — дёрнуть MCP/RPC
**Vault — кеш, не SSOT.** MCP `get_positions()` — единственный источник истины о текущих позициях.
Vault обновлять после каждого изменения позиций. Если `updated` старше 1 часа — перепроверить.
...
## Текущие позиции (03.07.2026 11:20 MSK — LIVE через MCP)

| Монета | Сторона | Плечо | Вход | Марк | PnL | SL |
|---|---|---|---|---|---|---|
| ALLO | SHORT | 10x | $0.3543 | $0.3448 | +$0.16 | $0.4054 |
| MOVE | LONG | 10x | $0.0124 | $0.0120 | -$12.01 | $0.0092 |
| STG | LONG | 10x | $0.1916 | $0.1569 | **-$72.90** | $0.15085 |
| WLFI | LONG | 10x | $0.0577 | $0.0573 | -$0.89 | $0.0532 |
| DOT | LONG | 10x | $0.8827 | $0.8565 | -$10.04 | $0.7673 |

**Итого: -$95.68** | 5 позиций
