...
**Состояние:** 23/23 пунктов роадмапа ✅. 10 тенантов. 5/5 фаз готовы. Продакшен-ready.
...
| Сервис | RAM | Роль |
|--------|-----|------|
| `apolaibot-demo.service` | ~62 MB | Demo-бот: каталог, онбординг, 2FA, фидбек, 10 команд |
...
### ✅ Сделано (23/23) — 22.06.2026

| Фаза | Пункты | Статус |
|------|--------|--------|
| 🔴 Безопасность | Агентская ФС, Podman-изоляция, 2FA, Audit trail | ✅ |
| 🟡 Монетизация | 4 тарифа, организации, bulk provisioning, промокоды | ✅ |
| 🟢 Продукт | Каталог, онбординг, пользовательские скиллы, vault, документы | ✅ |
| 🔵 Платформа | Облачное зеркало, очередь агентов, прозрачность тулов, реконнект | ✅ |
| ⚪ Масштаб | Health dashboard, whitelabel | ✅ |

### Новые инструменты (22.06)

`agent_fs.py`, `skill_sandbox.py`, `billing.py`, `tenant_features.py`, `user_skills.py`, `team_vault.py`, `doc_export.py`, `agent_queue.py`, `cloud_mirror.py`, `health_dashboard.py`, `whitelabel.py`
